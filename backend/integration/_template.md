...
> **Onde o contrato de cada feature fica.** O contrato de integração passa por
> dois estados: primeiro como **contrato-alvo** (`status: alvo`, o frontend
> constrói contra ele) e depois como **canônico** (`status: canônico`, o
> responsável de backend atualiza para refletir o código real). Preenchido à
> mão por feature, a partir deste modelo; preencha os campos `<...>`.
...
* **Auth**: cadastro com **CPF, e-mail e senha**; a senha é armazenada apenas
  ...
  ([RNF01](../../product/functional.md#requisitos-não-funcionais)). O
...
* **Pagamento**: integração com o gateway **AbacatePay** (Pix); ver
  [RF04](../../product/functional.md#rf04-efetuar-pagamento).
...
sozinha ([RN03](../../product/overview.md#rn03-expiração-da-reserva)),
...
Para cada rota (não listar apenas a rota, explicar o papel dela):

* **Método:** `<GET/POST/...>`
* **Path:** `<path>`
* **Tipo:** leitura / escrita
* **Finalidade:** `<papel da rota>`
* **Params de rota:** `<:id, ...>`
* **Pronta para o frontend?:** sim / não / parcial
