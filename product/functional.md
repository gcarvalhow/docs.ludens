...
por requisito**. Ver [visão geral do produto](overview.md) e
[regras de negócio](overview.md#regras-de-negócio-rn01rn05).
...
* O sistema valida o limite de ingressos por CPF ([RN01](overview.md#rn01-limite-de-ingressos-por-cpf))
  antes de confirmar a reserva.
* A reserva expira conforme [RN03](overview.md#rn03-expiração-da-reserva);
...
* Duas reservas simultâneas não podem exceder a capacidade da sessão (controle
  de concorrência; ver [RN05](overview.md#rn05-consistência-de-disponibilidade)).
...
* O sistema aplica a política de reembolso
  ([RN02](overview.md#rn02-política-de-reembolso)) automaticamente
...
* Nenhum dado de pagamento (dados bancários, identificadores do pagador) é
  ...
  (RF04, [visão geral do produto](overview.md)).
...
* O caminho feliz da compra (descoberta até confirmação) se conclui em no máximo
  **5 passos**.
...
* Interface responsiva, utilizável em tela de celular. Pré-requisito para a
  leitura de ingressos na portaria via dispositivo móvel, prevista como
...
* A aplicação sobe via **Docker** sem passos manuais além da definição das
  ...
  pipeline ([visão geral do produto](overview.md)).
...
* **RF01, RF02, RF08:** modelo de dados de catálogo (espetáculo, sessão,
  capacidade, valores) no PostgreSQL. Viabilização: Backend.
* **RF02, RF03, RF06:** consulta de disponibilidade em tempo real (leitura
  consistente do estoque da sessão). Viabilização: Backend.
* **RF03:** controle de concorrência atômico do estoque de ingressos
  ([RN05](overview.md#rn05-consistência-de-disponibilidade)) e expiração
  agendada da reserva ([RN03](overview.md#rn03-expiração-da-reserva)).
  Viabilização: Backend.
* **RF03:** validação do limite de ingressos por CPF
  ([RN01](overview.md#rn01-limite-de-ingressos-por-cpf)). Viabilização: Backend.
* **RF04, RF07:** integração com o gateway AbacatePay, cobrança e estorno via
  Pix. Viabilização: Backend + DevOps (secret da AbacatePay).
* **RF05, RF06, RF09:** serviço de e-mail transacional (confirmação, reenvio
  de ingresso, recuperação de senha). Viabilização: Backend + DevOps
  (credenciais SMTP).
* **RF05:** geração de identificador único / QR do ingresso. Viabilização:
  Backend.
* **RF09:** autenticação, *hash* de senha e sessão do usuário. Viabilização:
  Backend.
* **RF08, RF09:** controle de acesso, perfil administrador × comprador.
  Viabilização: Backend.
* **Todos:** ambiente Docker + PostgreSQL provisionado e pipeline com
  secrets configurados. Viabilização: DevOps.
